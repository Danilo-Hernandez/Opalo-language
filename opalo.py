...
KEYWORDS = ["var", "and", "or", "not", "if", "then", "elif", "else", "for", "to", "step", "while", "func", "end", "continue", "break", "return", "class"]
    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            if self.current_char in " \t":
                ...
                tokens.append(Token(TT_COLUMN, pos_start=self.pos))
                self.advance()
            elif self.current_char == "[":
                tokens.append(Token(TT_LSB, pos_start=self.pos))
                self.advance()
            elif self.current_char == "]":
                tokens.append(Token(TT_RSB, pos_start=self.pos))
                self.advance()
            elif self.current_char == '"':
...
class ClassDefNode:
    def __init__(self, var_name_tok, init_func, body_node):
        self.var_name_tok = var_name_tok
        self.body_node = body_node
        ...
        if self.body_node:
            ...
            self.pos_end = self.init_func.pos_end
...
class Parser:
    ...
    def statement(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'var', '[', 'if', 'for', 'while', 'func', int, float, identifier, '+', '-', 'break', 'continue', 'return' or '('"))
    ...
    def call_class(self):
        ...
        if self.current_tok.type == TT_LPAREN:
            ...
            if self.current_tok.type == TT_RPAREN:
                ...
                if res.error:
                    return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected ')', 'var', 'if', 'for', 'while', 'func', int, float, identifier, '+', '-' or '('"))
            ...
            return res.success(CallClassNode(atom, arg_nodes))
    ...
    def atom(self):
        ...
        return res.failure(InvalidSyntaxError(tok.pos_start, tok.pos_end, "Expected int, float, identifier, 'for', 'while', 'func', 'if', '+', '-', '[' or '('"))
    ...
    def term(self):
        return self.bin_op(self.factor, (TT_MUL, TT_DIV, TT_DOT))
    ...
    def comp_expr(self):
        ...
        node = res.register(self.bin_op(self.arith_expr, (TT_EE,TT_GT,TT_LT,TT_LTE,TT_GTE,TT_NE)))
    ...
    def class_def(self):
        ...
        if self.current_tok.type != TT_NEWLINE:
            return res.failure(
                InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, f"Expected newline"))
        ...
        node_to_return = res.register(self.func_def())
        if res.error: return res
        ...
        return res.success(ClassDefNode(var_name_tok, node_to_return, body))
    ...
    def func_def(self):
        ...
        if self.current_tok.type == TT_COLUMN:
            res.register_advancement()
            self.advance()
    ...
    def subbed_by(self, other):
        if isinstance(other, Number):
            ...
            try:
                ...
                return None, RTError(other.pos_start, other.pos_end, "Element at this index could not be removed from list beacause index is out of bounds", self.context)
        else:
            return None, Value.illegal_operation(other)
    ...
    def dived_by(self, other):
        if isinstance(other, Number):
            try:
                ...
                return None, RTError(other.pos_start, other.pos_end, "Element at this index could not be retrieved from list beacause index is out of bounds", self.context)
        else:
            return None, Value.illegal_operation(other)
...
class BaseFunction(Value):
    ...
    def check_args(self, arg_names, args):
        ...
        if len(args) > len(arg_names):
            return res.failure(RTError(self.pos_start, self.pos_end, f"{len(arg_names - len(args))} too many args passed into '{self.name}'", self.context))
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_pop(self, exec_ctx):
        ...
        try:
            ...
            return RTResult().failure(RTError(self.pos_start, self.pos_end, "Element at this index could not be removed because index is out of bounds"))
    ...
    def execute_round(self, exec_ctx):
        ...
        if not isinstance(num, Number):
            return RTResult().failure(RTError(self.pos_start, self.pos_end, "Argument must be a number", exec_ctx))

        return RTResult().success(Number(round(num.value)))
    execute_round.arg_names = ["num"]

    def execute_change_value(self, exec_ctx):
        list_ = exec_ctx.symbol_table.get("list")
        index = exec_ctx.symbol_table.get("index")
        value = exec_ctx.symbol_table.get("value")
        ...
        list_.elements[round(index.value)] = value
        return RTResult().success(Number.null)
    execute_change_value.arg_names = ["list", "index", "value"]

    def execute_rand_range(self, exec_ctx):
        ...
        return RTResult().success(Number(random.randrange(min.value, max.value, 1)))
    execute_rand_range.arg_names = ["min", "max"]
...
BuiltInFunction.change_value = BuiltInFunction("change_value")
BuiltInFunction.rand_range = BuiltInFunction("rand_range")
...
class ClassInstance(Value):
    ...
    def dotted_at(self, other):
        if isinstance(other, String):
            value = self.context.symbol_table.get(other.value)
            if value:
                return value.copy()
            else:
                return None, self.illegal_operation(other)
...
class Class(BaseFunction):
    def __init__(self, name, init_func, body_node):
        super().__init__(name)
        self.body_node = body_node
        self.init_func = init_func

    def copy(self):
        copy = Class(self.name,self.init_func, self.body_node)
        copy.set_pos(self.pos_start, self.pos_end)
        copy.set_context(self.context)
    ...
    def execute(self, args):
        ...
        self.init_func = res.register(interpreter.visit(self.init_func, new_context))
        if res.error: return res

        res.register(self.check_and_populate_args(self.init_func.arg_names, args, new_context))
        if res.should_return(): return res

        value = res.register(interpreter.visit(self.init_func.body_node, new_context))
        if res.error: return res

        ret_value = ClassInstance().set_context(new_context).set_pos(self.pos_start, self.pos_end)
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        if node.op_tok.type == TT_DOT:
            result, error = left.dotted_at(right)
    ...
    def visit_ForNode(self, node, context):
        ...
        if step_value.value >= 0:
            ...
            condition = lambda: 1 > end_value.value
    ...
    def visit_ClassDefNode(self, node, context):
        ...
        body_node = node.body_node
        class_value = Class(class_name, node.init_func, body_node).set_context(context).set_pos(node.pos_start, node.pos_end)
    ...
    def visit_CallClassNode(self, node, context):
        res = RTResult()
        args = []
...
global_symbol_table.set("change_value", BuiltInFunction.change_value)
global_symbol_table.set("rand_range", BuiltInFunction.rand_range)
